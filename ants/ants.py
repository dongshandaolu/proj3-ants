...
class QueenAnt(ScubaThrower):
    ...
    def construct(cls, gamestate):
        ...
        if gamestate.queen_exists:
            print('Only one queen can be created!')
            return None
    ...
    def __init__(self, health=1):
        """Create a QueenAnt with the given health."""
        super().__init__(health)
    ...
    def action(self, gamestate):
        ...
        while current_place is not None:
            if current_place.ant is not None:
                ant_to_buff = current_place.ant
                if ant_to_buff not in self.buffed_ants:

                    if ant_to_buff.is_container and ant_to_buff.ant_contained:
                        if ant_to_buff.ant_contained not in self.buffed_ants:
                            ant_to_buff.ant_contained.buff()
                            self.buffed_ants.add(ant_to_buff.ant_contained)

                    if ant_to_buff not in self.buffed_ants:
                        ant_to_buff.buff()
                        self.buffed_ants.add(ant_to_buff)
...
class Bee(Insect):
    ...
    def __init__(self, health, place=None):
        """Create a Bee with a health amount and a starting PLACE."""
        super().__init__(health, place)
        self.slow_length = 0        # 减速剩余回合数
        self.scare_length = 0       # 惊吓剩余后退次数
        self.has_been_scared = False
        self.direction = 'forward'  # 前进方向
    ...
    def action(self, gamestate):
        ...
        if self.scare_length > 0:
            ...
            if self.blocked() or gamestate.time % 2 == 1:
                pass
            else:
                self.scare_length -= 1
                if self.scare_length == 0:
        ...
        destination = self.place.exit if move_direction == 'forward' else self.place.entrance

        if move_direction == 'backward' and self.place.entrance is None:
            self.scare_length = 0
            self.direction = 'forward'
